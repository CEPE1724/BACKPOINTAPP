...
exports.uploadFileToGCSLatinium = async (req, res) => {
  try {
    ...
    if (!cedula || !tipo) {
      return res.status(400).json({ status: "error", message: "Se requiere 'cedula', 'tipo'." });
    }
    ...
    if (exists) {
      ...
      gcsFile = bucket.file(`${Directorio}/${tipo}/${cedula}/${newTimestamp}${path.extname(file.originalname)}`);
    }
