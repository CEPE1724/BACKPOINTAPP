...
exports.registrarSolicitudCredito = async (req, res) => {
 ...
 if (trabajo && Array.isArray(trabajo.trabajos) && trabajo.trabajos.length > 0) {
  // Si al menos un trabajo tiene fechaAfiliacionHasta === null, es dependiente
  const esDependiente = trabajo.trabajos.some((t) => t.fechaAfiliacionHasta === null);
  if (esDependiente) {
  ...
  try {
    ...
    if (!idCre_SolicitudWeb) {
      ...
      if (numeroSolicitud) {
        ...
        if (solicitud) {
          ...
          referenciasExistentes = await repoRef.find({ where: { idCre_SolicitudWeb } });
        }
      }
    ...
    if (cuotaAsignada >= precioCuotaCarrito && estadoFinal === 1) {
      if (idSituacionLaboral === 1) {
        // Dependiente
        mensaje = `¡Felicitaciones! Tienes un crédito pre aprobado, la cuota del carrito que escogiste al inicio es de $${precioCuotaCarrito}. Escoge las siguientes opciones.`;
...
function validarDireccion(direccion) {
  ...
  if (
    !(
      typeof direccion.Celular === "string" &&
      /^\d{10}$/.test(direccion.Celular)
    )
  ) {
    return {
      valido: false,
      mensaje: "El celular debe ser numérico y de 10 dígitos.",
    };
  }
  return { valido: true };
}
