...
exports.registrarDependiente = async (req, res) => {
  try {
    ...
    if (typeof direccionTexto !== "string") {
      return res.status(400).json({ mensaje: "La dirección debe ser un texto." });
    }
    if (direccionTexto.length > 499) {
      return res.status(400).json({ mensaje: "La dirección no puede exceder 499 caracteres." });
    }
    ...
    if (typeof celular !== "string" || !/^\d{10}$/.test(celular)) {
      return res.status(400).json({ mensaje: "El celular debe ser numérico y de 10 dígitos." });
    }
