...
exports.crearCarrito = async (req, res) => {
  ...
  let {
    ...
    Imagen,
    idWEB_Clientes,
    carritoDetalleSelIntangibleDto,
    idWEB_Grupo,
    PrecioCreditoDirecto } = carritoDetalle

  try {
    if (!idWEB_Carrito || idWEB_Carrito <= 0) {
      KeyCarrito = uuidv4()
    } else {
      const findKeyCarrito = await this.AppDataSource.query(
        'select wc.keyCarrito from WEB_Carrito wc where wc.idWEB_Carrito = @0',
        [idWEB_Carrito]
      )
      if (!KeyCarrito || KeyCarrito.length === 0) {
        return res.status(404).json({
          status: 'error',
    ...
    const detalleExistente = await this.AppDataSource.query(
      'select * from WEB_CarritoDetalle where idWEB_Categorias = @0 and Id = @1 and idWEB_Grupo = @2'
    )
  } catch (err) {
    return res.status(500).json({
      status: 'error',
